...
- 1.0.26에서 Button 매크로 추가로 실증 — host/v4-adapter, main.tsx 무수정 검증.

---

### ADR-018: Vite `target: 'es2019'` — esbuild helper 전역 누수 회피

**결정**: `vite.config.ts`의 `build.target`을 `'es2015'`가 아닌 **`'es2019'`로 설정**한다.

**이유**: 1.0.25에서 Button 매크로를 추가하며 번들 크기와 spread 사용량이 늘자 esbuild minify가 object spread polyfill을 다음 형태로 emit:
```js
var qm=Object.defineProperty,ev=Object.defineProperties;
var tv=Object.getOwnPropertyDescriptors;
// ...
var $=(Te,oe)=>ev(Te,tv(oe));   // ← object spread helper
```
이 `var` 선언들이 **rollup IIFE 바깥**(전역 script scope)에 hoist되어 `window.$`에 들러붙음 → 후속 batch.js의 모든 jQuery 호출(`$.extend`, `$(document).ready` 등)이 깨짐 → `$.extend is not a function` + `Cannot convert undefined or null to object` 연쇄로 Confluence Blueprint, MV.AnchorManager, spacesidebar.js 등 인티 함수 줄줄이 실패 → 편집 페이지 dead.

`target: 'es2019'`이면 esbuild이 object spread를 native로 인식해 helper 자체를 emit 안 함. ES2018부터 object spread가 표준이지만 안전 마진으로 ES2019(esbuild target에서 가장 보편적인 모던 baseline) 선택.

**대안 검토**:
- 전체 번들을 추가 IIFE로 wrap (`banner: '(function(){', footer: '})();'`) → 시도했으나 esbuild minify 단계에서 `var` 선언이 banner 위로 hoist되어 효과 없음.
- minify를 esbuild → terser로 교체 → terser는 hoist 안 하지만 빌드 시간 증가 + 번들 size 비슷.
- 결국 가장 깔끔한 해결: target만 올려 helper 자체 미생성.

**브라우저 호환성**: Confluence DC 7.19.8 공식 지원 브라우저(Chrome 78+, Firefox 80+, Edge 78+, Safari 13+)는 모두 ES2019 native. IE는 7.19부터 미지원이라 무관.

**금지 사항**:
- `target`을 'es2015' / 'es2017' 등 ES2018 미만으로 낮추지 말 것 — 동일 회귀 재발.
- 새 helper 누수 의심 시 검증: `head -c 100 dist/wonikips-editor.js`가 `var ` 또는 `const ` 같은 선언이 아닌 `(function` / `typeof process` 등으로 시작해야 정상.
...
**결정**: `getBodyType() == RICH_TEXT`인 매크로(현재 Panel + BackgroundImage, 추후 Tab / TabCollection)의 자체 패널 opener에서 `tinymce.confluence.macrobrowser.macroBrowserComplete(...)` 호출 시 `bodyHtml: ''`을 **반드시 포함**한다. NONE body 매크로(Cards/Button/Divider)는 생략 가능.
...
**적용 범위**: Panel(1.1.5 fix), BackgroundImage(1.1.8 — 첫 빌드부터 적용, 회귀 없이 지나감). 향후 Tab/TabCollection 자체 패널화 시 동일 패턴 적용.
...
**적용 범위**: Panel(1.1.4). 향후 같은 nested 패턴이 필요한 매크로(TabCollection의 panel-like config 등)에 재사용. **BackgroundImage(1.1.8)는 적용 대상 아님** — Java `BackgroundImage.executeWeb`이 `map.get("backgroundColor")`/`map.get("containerMinHeight")` 등 평탄 키를 직접 읽고 serialized JSON wrapper를 사용하지 않기 때문에 Cards/Button/Divider와 같은 평탄 K=V mapper 패턴을 따른다. nullable 필드(`backgroundColor`/`backgroundImageHref`)는 mapper에서 키 자체 omit해 Java의 `map.containsKey(...)` 분기와 맞춘다.
