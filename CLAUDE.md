...
## 자체 React 매크로 편집 패널 (Cards + Button, 1.0.26에서 동작 검증)

Aura main.js 대체용 자체 React 패널 (`src/main/resources/client-custom/`). **Cards (1.0.23) + Button (1.0.26)** 우리 자체 패널로 동작. 1.0.24에서 registry 패턴 인프라 완료 — 새 매크로는 `src/macros/{name}.ts` + `src/editors/{Name}Editor/` + `src/schema/{name}*.ts` + `src/macros/index.ts` 한 줄 추가만으로 등록 (host/v4-adapter, main.tsx 무수정). 다른 매크로(Panel/Tab/Title/...)는 여전히 Aura main.js 사용 — 같은 패턴으로 점진 전환.
...
`host/v4-adapter.ts`, `main.tsx`, Java 매크로 클래스 무수정. 1.0.26 Button 추가가 이 패턴 실증.

### 상세 계획 + 회귀 기록
`docs/CUSTOM_PANEL_PLAN.md` (4주 일정 + 검증 결과), `docs/PARALLEL_DEV_GUIDE.md` (worktree 병렬 작업), `docs/ADR.md` (ADR-015~018), `docs/DEVELOPMENT_GUIDE.md` (전체 가이드).
